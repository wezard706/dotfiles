...
10. 検証済みスキルを`~/.agents/skills`と`~/.claude/skills`へ反映する。
...
含まれる管理対象だけを、スキル単位で置換する。

既存の`cp -r`による重ね書きでは削除済みファイルが残るため、管理対象スキルの
置換時は新しいディレクトリを隣接する一時名へコピーし、コピー完了後に対象名へ
切り替える。Codex用とClaude Code用の片方で反映に失敗した場合は非0で終了し、
どちらへの反映で失敗したかを表示する。
