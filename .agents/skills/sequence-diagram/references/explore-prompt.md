...
- ジョブのenqueue: 記録する。**ジョブ本体（perform内部）のトレースも同じJSON内に続けて記録する**（メインが同じシーケンス図にまとめて描画するため）
      ...
      "steps": [
        {
          ...
          "kind": "call",
          "label": "perform",
          "description": "ジョブ本体を実行",
          "file": "app/jobs/welcome_email_job.rb:6"
        },
        {
          "from": "Job",
          "to": "Mailer",
          "kind": "call",
          "label": "welcome_email(user).deliver_now",
          "description": "ウェルカムメールを送信",
          "file": "app/jobs/welcome_email_job.rb:8"
        }
      ],
      "note": "enqueue ステップの後に同じシーケンス図の続きとして描画される"
    }
  ],
  "alt_branches": [
    {
...
- **非同期処理は `async_branches` に記録するが、ジョブ本体（perform内部）の steps も同じJSON内に含める**。メインはこれを enqueue ステップに続けて同じシーケンス図に描画する
