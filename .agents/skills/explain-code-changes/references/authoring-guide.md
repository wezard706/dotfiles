...
├── code-review.md         code-review スキルの出力をそのまま保存する
└── chapters/
    ├── summary.html       省略不可
...
`--code-review` は必須。`code-review` スキルの出力をそのまま保存したファイルを渡す。空ファイルではビルドが失敗する。内容はレポートのフィードバック章へ折りたたんで添付され、レビュワーが指摘の元をたどれるようにする。
