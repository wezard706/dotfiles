...
1. 基準refを決める。指定がなければリポジトリの既定ブランチ（`origin/HEAD` が指す先）が使われる。解決できなければ質問する。
   決めたら `git fetch` で最新化する。古い基準refのままだと、既に上流へ取り込まれた変更まで差分に混ざり、このブランチの変更として解説してしまう。

   ```bash
   git fetch origin
   ```

   リモートが無い・オフライン等で失敗しても続行してよい。その場合はローカルrefが基準になるので、レポートの解説でその前提に触れる必要はないが、差分が想定より広いときは基準refの鮮度を疑う。
