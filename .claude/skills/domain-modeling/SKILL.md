...
| ドメインモデル図（集約ルート・エンティティ・列挙型） | Ruby等のクラス・メソッド・ファイル配置 |
| 各図の理想案と現実案 | 値オブジェクト（クラスとして描かず属性に展開する） |
| | 現行モデルとの差分表 |
| | ユビキタス言語・用語集（`ubiquitous` スキルの担当） |
| | 境界づけられたコンテキスト（BC）の分割・境界判断 |

**BCは扱わない。** 集約とエンティティという戦術的パターンだけでモデルを組む。新BCが必要か等の戦略的設計は、このスキルの対象外。

現行モデルは**内部利用のみ**。現行モデル図をファイルに出力することはしない。成果物に描くのは常に「機能追加後の状態」。

**図の主役は新しく登場する概念。** 既存概念は、新概念がどこに接続するのかを示すために必要な分だけ描く。調査した既存モデルを全部描くと、新概念が既存の海に埋もれて図の価値がなくなる。詳細は `references/domain-model-notation.md` の「図の主役は新しく登場する概念」を参照。
...
- 集約の切り方、概念の置き方 → 理想案/現実案として提示する対象
- 命名 → ドラフトして提示すればよい
- 「理想案と現実案どちらがいいですか」 → 判断材料を並べるのがこのスキルの仕事で、選ばせるのは最後

質問は**Step 1 でまとめて出す**。`AskUserQuestion` を使う。聞くことがなければ、質問せず Step 2 へ進む。

### Step 2: 現行モデルを把握する

既存コードを自律的に探索し、機能追加が触れる範囲の現行モデルを把握する。ユーザーに探索範囲の確認は取らない。

`Explore` サブエージェントに委譲する。メインコンテキストでは直接 Grep/Read しない。
...
**集めた概念は2つに仕分けて渡す。** この仕分けをせずに調査結果をそのまま渡すと、現実側エージェントは調べた既存概念を全部図に描いてしまい、新概念が埋もれる。

- **接続点**: 新機能が直接触れる、または直接関係を持つ既存概念。これは図に描かれる候補
- **背景**: 上記を理解するために調べたが、新機能とは直接つながらない既存概念。図には描かず、判断の材料としてだけ使う

調査そのものは広くてよい。狭めるのは描く量であって、理解の範囲ではない。
...
- ドメインモデル図: `references/domain-model-notation.md` — 既存概念をどこまで描くか、ステレオタイプ、NG/OK例
