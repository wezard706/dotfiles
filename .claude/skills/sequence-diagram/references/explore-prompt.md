...
以下のコードからシーケンス図を生成するために、呼び出しグラフを構造化して返してください。
  ...
  "alt_branches": [
    {
      ...
      "then_steps": [
        { "from": "Service", "to": "Mutation", "kind": "raise", "label": "ValidationError" }
      ],
      "else_steps": []
    }
  ],
...
- **条件分岐は `alt_branches` に分離**。メインがMermaidの`alt`ブロックに変換する
